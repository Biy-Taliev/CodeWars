...
Your family runs a shop and have just brought a Scrolling Text Machine (http://3.imimg.com/data3/RP/IP/MY-2369478/l-e-d-multicolour-text-board-250x250.jpg) to help get 
some more business.
...
Your father is far too busy with the business to worry about such details, 
...
def rotate(str_):
    ...
    if str_:
        ...
        rotate = str_
        start = rotate[1:]+rotate[0]

        l = []

        while start != str_:
            l.append(start)
            next = start[1:]+start[0]
            start = next
